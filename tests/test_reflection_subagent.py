...
def _seed_session(session_store: SessionStore, bug_id: str) -> None:
    session_store.put(
        ...
        {
            "base_branch": "main",
            "agent_branch": "agent-fix/demo",
            "artifact_paths": {},
            "trace_full_sanitized.log": "trace",
            "trace_frames.json": "[]",
            "tool_calls.jsonl": "[]",
            "agent_patch.diff": "diff",
            "compile_result.json": "{}",
            "test_result.json": "{}",
        },
    )
    session_store.put(
        f"bug_event:{bug_id}",
        {"bug_id": bug_id, "source": "feishu", "project": "demo", "title": "NPE", "exception_type": "NullPointerException", "message": "x", "traceback": "", "fingerprint": "fp"},
    )


def test_reflection_review_passed_creates_skill(tmp_path) -> None:
    """review_passed 应生成 skill 并写入存储。"""
...
def test_reflection_review_failed_requires_human_branch(tmp_path) -> None:
    """review_failed 缺少 human_fix_branch 时必须报错。"""
    session_store = SessionStore()
    skill_store = SkillStore()
    bug_id = "bug-2"
    _seed_session(session_store, bug_id)

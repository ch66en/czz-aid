...
class ReflectionSubAgent:
    ...
    def __init__(
        ...
        self.feishu_tool = feishu_tool or FeishuTool(dry_run=True)
        self.git_tool = git_tool or GitTool(self.config.workspace)
    ...
    def _handle_review_passed(self, review_event: ReviewEvent) -> ReflectionResult:
        ...
        prompt = self._build_summary_prompt(mode="review_passed", bug_event=bug_event, review_event=review_event, trace_full=trace_full, trace_frames=trace_frames, tool_calls=tool_calls, agent_patch=agent_patch, compile_result=compile_result, test_result=test_result)
    ...
    def _handle_review_failed(self, review_event: ReviewEvent, payload: dict[str, Any]) -> ReflectionResult:
        ...
        prompt = self._build_summary_prompt(mode="review_failed", bug_event=bug_event, review_event=review_event, trace_full=self._load_artifact_text(session, "trace_full_sanitized.log"), trace_frames=self._load_artifact_json(session, "trace_frames.json"), tool_calls=self._load_artifact_text(session, "tool_calls.jsonl"), agent_patch=agent_diff, compile_result=self._load_artifact_json(session, "compile_result.json"), test_result=self._load_artifact_json(session, "test_result.json"), extra={"human_fix_branch": human_fix_branch, "human_diff": human_diff, "diff_summary": diff_summary.summary})
        summary = self._ask_llm(prompt)
        skill_artifact = self._generate_and_persist_skill(bug_event=bug_event, review_event=review_event, title=bug_event.title or bug_event.exception_type, body=summary)
    ...
    def _generate_and_persist_skill(self, *, bug_event: BugEvent, review_event: ReviewEvent, title: str, body: str) -> SkillArtifact:
        ...
        self._update_session(review_event.task_id, {"skill_name": skill_name, "skill_dir": str(skill_dir), "skill_meta": artifact.meta.model_dump()})
        return artifact
    ...
    def _git_diff(self, base_branch: str, target_branch: str) -> str:
        completed = subprocess.run(["git", "-C", self.config.workspace, "diff", f"{base_branch}...{target_branch}"], capture_output=True, text=True, shell=False, check=False)
        if completed.returncode == 0:
            return (completed.stdout or "").strip()
        result = self.git_tool.run({"action": "diff", "args": {}})
        return result.stdout_summary or f"diff({base_branch}...{target_branch}) unavailable"
